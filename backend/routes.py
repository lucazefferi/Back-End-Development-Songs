...
@app.route('/song')
def song():
    #db.songs.find({}) ritorna un cursore
    #che viene trasformato in lista con list()
    results = list(db.songs.find({}))
    #con parse_json trasformo results in un OBJ json passabile nella response
...
def get_song_by_id(id):
    ...
    if result is not None:
        return {"song": parse_json(result)}, 200
    else:
